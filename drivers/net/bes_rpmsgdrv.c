...
#include <nuttx/net/bes_rpmsgdrv.h>
 ...
 * Private Types
 ****************************************************************************/
extern void rndis_router_setup(FAR const char *ifname);
extern void rndis_router_teardown(void);
extern void bes_set_lan_dripv6addr(FAR const char *ifname);
extern void bes_set_lan_dripv6dns(FAR const char *ifname);
                                  ...
                                  FAR netpkt_t *pkt)
{
  ...
  if (len < sizeof(*transfer) + datalen)
    {
      nerr("ERROR: Buffer is too small for xmit, %ld, %d, %d\n",
           len, datalen, sizeof(*transfer));
...
net_rpmsg_drv_receive(FAR struct netdev_lowerhalf_s *dev)
{
  ...
  iob = netpkt_remove_queue(&drv->rxqueue);
  return iob;
                                          ...
                                          uint32_t src, FAR void *priv)
{
  ...
  if (transfer->length > len - sizeof(*transfer))
    {
      _err("ERROR: net_rpmsg got invalid transfer length!");
      goto drop;
    }
  ...
  if (pkt == NULL)
    {
      _err("ERROR: Failed to allocate buffer!\n");
      goto drop;
    }
  ...
  if (netpkt_copyin(dev, pkt, (FAR uint8_t *)(transfer + 1),
                    transfer->length, 0) < 0)
    {
      _err("ERROR: Failed to copyin data!\n");
      goto free;
    }

  if (netpkt_tryadd_queue(pkt, &drv->rxqueue) < 0)
    {
      _err("ERROR: Failed to add pkt to queue!\n");
      goto free;
    }
...
#if defined(CONFIG_NET_MCASTGROUP) || defined (CONFIG_NET_IGMP)
static int net_rpmsg_drv_addmac(FAR struct netdev_lowerhalf_s *dev,
                                FAR const uint8_t *mac)
{
...
net_rpmsg_drv_alloc(FAR const char *devname, enum net_lltype_e lltype)
{
  ...
  lltype = NET_LL_ETHERNET;
  netdev = &drv->dev;
  netdev->quota[NETPKT_RX] = CONFIG_IOB_NBUFFERS /
                             NET_RPMSG_DRV_MAX_NIOB / 2;
...
static uint8_t g_mac_addr[6] =
{
  ...
  0x02, 0x00, 0xEE, 0x11, 0x22, 0x33
#else
  0x02, 0x00, 0x55, 0x11, 0x22, 0x33
#endif
};
#if 0
...
#endif
  memcpy(&netdev->netdev.d_mac.ether.ether_addr_octet, g_mac_addr, 6);
...
#if defined(NET_RPMSG_ROLE_GATEWAY) || defined(CONFIG_BES1700_AP)
  /* Tmp net config code*/
  {
    struct in_addr addr;
...
#if defined(NET_RPMSG_ROLE_GATEWAY) || defined(CONFIG_BES1700_APC1)
  /* Tmp net config code*/
  {
    struct in_addr addr;
...
#if defined(CONFIG_NET_RPMSG_DRV_SERVER) || \
    defined(CONFIG_BES1700_AP)           || \
    defined(CONFIG_BES3601P_AP)
/****************************************************************************
